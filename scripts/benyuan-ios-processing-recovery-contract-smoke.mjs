...
assert.match(flowModel, /restoreActiveGenerationJobIfNeeded[\s\S]*client\.fetchNativeGenerationJob/, "restore should fetch the current cloud job before polling");
assert.match(flowModel, /applyNativeGenerationJob\(job,\s*source:\s*\.restore/, "restore should apply a restore-specific processing message");
assert.match(flowModel, /max\(processingProgress,\s*min\(max\(job\.progress/, "job polling should keep processing progress monotonic");
assert.match(flowModel, /job\.canResumeInBackground/, "native flow should surface whether the cloud job can continue in background");
...
assert.match(flowModel, /后台继续运行/, "native processing copy should explicitly say cloud generation continues in background");
assert.match(processingView, /model\.activeGenerationJobId/, "processing view should show a concrete resumable job marker");
assert.match(processingView, /云端任务/, "processing view should label the cloud job state");
