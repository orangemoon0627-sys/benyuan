...
function RitualRunner({ phase, onRetry }: { phase: RitualPhase; onRetry: () => void }) {
  ...
  const [body, setBody] = useState(
    phase === "part1"
      ? "这一步不再是假加载。你会经历真实 API 的保存、多模态分析和导演生成，然后直接进入 Part 2。"
      : "现在系统会先写入 Part 2 选择，再调用分析师 Agent 生成完整星图，最后跳转到 Part 3。",
  );
  ...
  useEffect(() => {
    ...
    async function runPart1Flow() {
      ...
      if (!pending) {
        ...
        setBody("请先从特征收集页完成 A / B / C 模块，processing 页面才会开始真实调用多模态分析与导演 Agent。");
      ...
      setBody("先保存 Part 1，再调多模态和导演 Agent。处理完成后会自动进入剧场，不需要你继续等待在问卷页。");
    ...
    async function runConstellationFlow() {
      ...
      if (!pending) {
        ...
        setBody("请先完成剧场体验并提交选择记录，processing 页面才会开始真实调用分析师 Agent。");
      ...
      setBody("这一步会先把你的行动和镜像选择写回，再调用分析师 Agent 生成最终星图；完成后自动跳转。");
  ...
  }, [activeElapsedSeconds, activeStageStartedAt, stages]);
    ...
    <div className="grid gap-6">
      <GlassPanel className="relative overflow-hidden bg-[radial-gradient(circle_at_top,rgba(212,175,55,0.12),rgba(0,0,0,0.96)_62%)]">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <MetaPill><Clock3 className="mr-1 inline h-3 w-3" />预计剩余 {formatSeconds(remainingSeconds)}</MetaPill>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.35fr_0.65fr]">
        <GlassPanel>
          <SectionTitle label="Pipeline" title="现在系统真实走到哪一步，你可以直接看到。" description="不是纯 loading 文案，而是按阶段推进的真实前端编排。" />
        ...
        <GlassPanel>
          <SectionTitle label="State" title={emptyStateHref ? "当前还没有待处理任务。" : error ? "这次处理中断了。" : running ? "正在稳定显影。" : "已经准备跳转。"} />
          <div className="space-y-4 text-sm leading-7 text-[var(--text-secondary)]">
            <p>
              {error
                ? "你的输入没有丢。刷新页面后会从已完成阶段继续，而不是整段从头重跑。"
                : phase === "part1"
                  ? "处理完成后会直接带着 theater_script 进入剧场，并保留 runtime 与时间信息。"
                  : "处理完成后会直接打开精神星图页，并把本次分析运行时一起带过去。"}
            </p>
            <div className="border border-[var(--border)] bg-[rgba(255,255,255,0.012)] p-4">
              <p className="text-[11px] uppercase tracking-[0.26em] text-[var(--text-tertiary)]">live runtime</p>
            ...
            <div className="border border-[var(--border)] bg-[rgba(255,255,255,0.012)] p-4">
              <p className="text-[11px] uppercase tracking-[0.26em] text-[var(--text-tertiary)]">resume policy</p>
              <p className="mt-3 text-sm leading-7 text-[var(--text-secondary)]">
                当前阶段与已完成结果会写进本地缓存。即使刷新，只要 pending 数据还在，系统就会直接从下一步继续跑。
                {resumedFromCheckpoint ? " 你现在看到的就是一次从检查点恢复后的继续处理。" : ""}
              </p>
            </div>
            {error ? (
              <button
