...
export function BenyuanTheaterExperience() {
  ...
  const totalHoverCount = choiceLogs.reduce((total, item) => total + (item.hover_sequence?.length ?? 0), 0);
  ...
  const phaseHint = useMemo(() => {
    switch (phase) {
      case "act1":
        return "先沉进场景，再进入下一幕；这一段更像吸一口气，而不是立刻做决定。";
      case "act2":
        return `系统正在记录第二幕的分支选择与 hover 轨迹（${act2Progress}）。`;
      case "act3":
        return `系统正在记录第三幕的镜像回答（${act3Progress}）。`;
      case "epilogue":
        return "所有选择都已经落盘，现在只差把它们折成完整星图。";
      default:
        return "";
    }
  }, [act2Progress, act3Progress, phase]);
  ...
  if (loading) {
    return (
      <GlassPanel>
        <SectionTitle label="剧场装载" title="正在调入你的剧场脚本。" description="导演脚本一旦就位，这里就会接住你。" />
      </GlassPanel>
    );
  }
  ...
  if (!record) {
    return (
      <GlassPanel>
        <SectionTitle label="暂无剧场脚本" title="还没有可体验的剧场脚本。" description="先去 Part 1 完成输入，系统生成 theater_script 后再回来。" />
      </GlassPanel>
    );
  }
    ...
    <div className="grid gap-6">
      <GlassPanel className="overflow-hidden bg-[radial-gradient(circle_at_top,rgba(212,175,55,0.1),rgba(0,0,0,0.96)_58%)]">
        <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-[11px] uppercase tracking-[0.38em] text-[var(--text-tertiary)]">剧场运行时</p>
            ...
            <p className="mt-4 max-w-3xl text-sm leading-7 text-[var(--text-secondary)] md:text-base">
              导演 Agent 会把你的审美、哲学和生命线索折叠成一段可体验的戏剧路径；系统会同步记录你的选择、迟疑与停顿，并把这些痕迹继续写回同一条分析链路。
            </p>
          </div>
        ...
        <div className="mt-6 h-px overflow-hidden bg-[rgba(255,255,255,0.1)]">
          <div className="h-full bg-[linear-gradient(90deg,rgba(212,175,55,0.2),rgba(212,175,55,1))]" style={{ width: `${phaseProgressPercent}%` }} />
        </div>

        <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <div className="border border-[var(--border)] bg-[rgba(255,255,255,0.018)] px-4 py-4">
            <p className="text-[11px] uppercase tracking-[0.32em] text-[var(--text-tertiary)]">当前章节</p>
            <p className="mt-4 text-lg text-[var(--text-primary)]">{phaseMeta[phaseIndex]?.label ?? "--"}</p>
            <p className="mt-3 text-sm leading-7 text-[var(--text-secondary)]">{phaseMeta[phaseIndex]?.title ?? "--"}</p>
          </div>
          <div className="border border-[var(--border)] bg-[rgba(255,255,255,0.018)] px-4 py-4">
            <p className="text-[11px] uppercase tracking-[0.32em] text-[var(--text-tertiary)]">已记录选择</p>
            <p className="mt-4 text-lg text-[var(--text-primary)]">{choiceLogs.length} / {record.theater_script.act2.choices.length}</p>
            <p className="mt-3 text-sm leading-7 text-[var(--text-secondary)]">第二幕的分支选择已写回本地轨迹。</p>
          </div>
          <div className="border border-[var(--border)] bg-[rgba(255,255,255,0.018)] px-4 py-4">
            <p className="text-[11px] uppercase tracking-[0.32em] text-[var(--text-tertiary)]">已记录镜像</p>
            <p className="mt-4 text-lg text-[var(--text-primary)]">{mirrorLogs.length} / {record.theater_script.act3.mirror_questions.length}</p>
            <p className="mt-3 text-sm leading-7 text-[var(--text-secondary)]">第三幕的镜像回答会继续送往分析师。</p>
          </div>
          <div className="border border-[var(--border)] bg-[rgba(255,255,255,0.018)] px-4 py-4">
            <p className="text-[11px] uppercase tracking-[0.32em] text-[var(--text-tertiary)]">Hover 轨迹</p>
            <p className="mt-4 text-lg text-[var(--text-primary)]">{totalHoverCount}</p>
            <p className="mt-3 text-sm leading-7 text-[var(--text-secondary)]">系统累计记录到的试探与犹豫次数。</p>
          </div>
        </div>

        <div className="mt-5 border border-[rgba(212,175,55,0.18)] bg-[rgba(212,175,55,0.05)] px-4 py-4 text-sm leading-7 text-[var(--text-secondary)]">
          {status || phaseHint}
        </div>
      </GlassPanel>
      ...
      {phase === "epilogue" ? (
        <GlassPanel className="min-h-[68vh] bg-[radial-gradient(circle_at_top,rgba(212,175,55,0.12),rgba(0,0,0,0.96)_58%)]">
          <div className="mx-auto flex max-w-3xl flex-col items-center justify-center py-8 text-center">
            ...
            <p className="mt-4 text-sm text-[var(--text-secondary)]">{status || "会把选择记录、hover 轨迹与阶段耗时一并写回当前分析链路。"}</p>
          </div>
        </GlassPanel>
      ) : null}
    </div>
