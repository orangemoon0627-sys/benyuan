...
import { getDraftSession, getDraftWorkflowSummary, simulateDraftApplication, syncWorkbenchDraftSessions } from "@/lib/store";
...
export async function GET(_: Request, { params }: { params: Promise<{ draftId?: string[] }> }) {
  ...
  return NextResponse.json({
    ...
    draft,
    simulation: await simulateDraftApplication(draftId),
