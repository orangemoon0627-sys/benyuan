...
function ConstellationAtlas({ dimensions, archetype, mode }: { dimensions: ConstellationDimensionReading[]; archetype: ReportPayload["archetype"]; mode: ReportViewMode }) {
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[0.56fr_0.44fr]">
      ...
      >
        ...
        <div className="relative mx-auto aspect-square w-full max-w-[31rem]">
          ...
          <div className="absolute left-1/2 top-1/2 flex h-36 w-36 -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center rounded-full bg-[radial-gradient(circle,rgba(255,255,255,0.16),rgba(255,255,255,0.05)_50%,transparent_78%)] text-center shadow-[0_0_60px_rgba(191,222,255,0.16)]">
            <p className="text-[10px] uppercase tracking-[0.28em] text-stone-400">current core</p>
      ...
      >
        <p className="text-[11px] uppercase tracking-[0.32em] text-stone-500">constellation summary</p>
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          <div className="rounded-[24px] bg-white/[0.03] px-4 py-4 shadow-[0_0_0_1px_rgba(255,255,255,0.05)]">
            <p className="text-[10px] uppercase tracking-[0.24em] text-stone-500">average glow</p>
          ...
          <div className="rounded-[24px] bg-white/[0.03] px-4 py-4 shadow-[0_0_0_1px_rgba(255,255,255,0.05)]">
            <p className="text-[10px] uppercase tracking-[0.24em] text-stone-500">strongest axis</p>
          ...
          <div className="rounded-[24px] bg-white/[0.03] px-4 py-4 shadow-[0_0_0_1px_rgba(255,255,255,0.05)]">
            <p className="text-[10px] uppercase tracking-[0.24em] text-stone-500">quietest axis</p>
        ...
        <div className="mt-6 rounded-[28px] bg-white/[0.03] p-5 shadow-[0_0_0_1px_rgba(255,255,255,0.05)]">
          <p className="text-[11px] uppercase tracking-[0.28em] text-stone-500">top constellation forces</p>
  ...
  return (
    ...
    >
      ...
      <div className="mt-5 space-y-4">
        {items.map((item) => (
          <div key={`${group}-${item.title}`} className="rounded-[22px] bg-white/[0.03] px-4 py-4 shadow-[0_0_0_1px_rgba(255,255,255,0.05)]">
          ...
          <section className="relative overflow-hidden rounded-[44px] bg-[linear-gradient(135deg,rgba(153,117,124,0.22),rgba(185,215,246,0.12),rgba(255,255,255,0.03))] px-7 py-8 shadow-[0_0_0_1px_rgba(255,255,255,0.06)] backdrop-blur-2xl md:px-10 md:py-10">
            ...
            <div className="relative grid gap-8 lg:grid-cols-[1.08fr_0.92fr] lg:items-end">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                {report.analysisMeta ? (
                  <div className={cx("rounded-[28px] p-5 shadow-[0_0_0_1px_rgba(255,255,255,0.06)]", mode === "immersive" ? "bg-black/18" : "bg-[rgba(189,218,255,0.08)]")}>
                    <p className="text-[11px] tracking-[0.3em] text-stone-500 uppercase">analysis runtime</p>
                    <p className="mt-3 text-base leading-7 text-stone-100">{report.analysisMeta.engineLabel}</p>
                    <p className="mt-2 text-sm leading-7 text-stone-400">
                      {report.analysisMeta.providerKind} / {report.analysisMeta.providerModel ?? "deterministic-only"}
                    </p>
                    {report.analysisMeta.providerEnhancementStatus ? (
                      <p className="mt-2 text-xs leading-6 text-stone-500">
                        enhancement {report.analysisMeta.providerEnhancementStatus}
                    ...
                    {report.analysisMeta.providerCompletedScopes?.length ? (
                      <p className="mt-1 text-xs leading-6 text-stone-500">
                        scopes: {report.analysisMeta.providerCompletedScopes.join(" · ")}
                      </p>
                    ) : null}
                    {typeof report.analysisMeta.providerTextReceived === "boolean" ? (
                      <p className="mt-1 text-xs leading-6 text-stone-500">stream text: {report.analysisMeta.providerTextReceived ? "received" : "empty"}</p>
                    ) : null}
                    {report.analysisMeta.providerResponsePreview ? (
                      <p className="mt-1 line-clamp-3 text-xs leading-6 text-stone-500">preview: {report.analysisMeta.providerResponsePreview}</p>
                    ) : null}
                    {report.analysisMeta.providerFallbackReason ? (
                      <p className="mt-1 text-xs leading-6 text-stone-500">fallback: {report.analysisMeta.providerFallbackReason}</p>
                    ) : null}
                  </div>
                ) : null}
              </div>
        ...
        <section className="mt-12 grid gap-12 xl:grid-cols-[0.28fr_0.72fr] xl:items-start">
          ...
          <div className="space-y-8 md:space-y-10">
            ...
            <MotionReveal delay={0.16}>
              <SectionShell
                id="dimensions"
                eyebrow={hasConstellation ? "02 / constellation" : "02 / dimensions"}
                        ...
                        >
                          ...
                          <div className="relative">
                            <div className="flex items-start justify-between gap-4">
                              <div>
                                <p className="text-[11px] tracking-[0.3em] text-stone-500 uppercase">{dimension.key.replaceAll("_", " ")}</p>
                        ...
                        <article key={reading.dimension} className={cx("rounded-[28px] p-5 transition duration-300 md:p-6", mode === "immersive" ? "bg-black/16 shadow-[0_0_0_1px_rgba(255,255,255,0.06)]" : "bg-[rgba(189,218,255,0.08)] shadow-[0_0_0_1px_rgba(189,218,255,0.12)]")}>
                          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                            <div className="max-w-2xl">
                              <p className="text-[11px] tracking-[0.32em] text-stone-500 uppercase">0{index + 1} / legacy channel</p>
            ...
            <MotionReveal delay={0.2}>
              <SectionShell id="tensions" eyebrow="03 / tensions" title="核心张力" lead="重要的不是消除张力，而是知道自己正被哪两股力量同时拉住，以及如何让它们在现实里继续共存。" mode={mode}>
                <div className="space-y-5">
                    ...
                    {report.archetype.visualPrompt ? (
                      <div className={cx("rounded-[24px] p-5 shadow-[0_0_0_1px_rgba(255,255,255,0.05)]", mode === "immersive" ? "bg-white/[0.03]" : "bg-[#11151d]/75")}>
                        <div className="flex items-center gap-3 text-stone-300">
                          ...
                          <p className="text-[11px] tracking-[0.28em] text-stone-500 uppercase">visual seed</p>
                        </div>
            ...
            {hasGrowthSuggestions ? (
              <MotionReveal delay={0.28}>
                <SectionShell id="growth" eyebrow="05 / growth direction" title="接下来可以怎样生长" lead="这些建议不是标准答案，而是依据你当前的结构，为现实生活预留出的几条更贴身的下一步。" mode={mode}>
                  <div className="grid gap-4 xl:grid-cols-2">
                    {report.growthSuggestions?.map((item, index) => (
                      <article key={`${item.title}-${index}`} className={cx("rounded-[28px] p-5 shadow-[0_0_0_1px_rgba(255,255,255,0.06)] md:p-6", mode === "immersive" ? "bg-black/16 hover:bg-white/[0.035]" : "bg-[rgba(189,218,255,0.08)]")}>
                        <p className="text-[11px] tracking-[0.3em] text-stone-500 uppercase">growth note</p>
                        ...
                        <div className="mt-5 space-y-3">
                          {item.actionableSteps.map((step) => (
                            <div key={step} className="rounded-[20px] bg-white/[0.03] px-4 py-3 text-sm leading-7 text-stone-200/84 shadow-[0_0_0_1px_rgba(255,255,255,0.05)]">
                              {step}
                            </div>
                          ))}
                        </div>
                ...
                {hasCuratedRecommendations ? (
                  <div className="space-y-8">
                    <div className="grid gap-4 xl:grid-cols-3">
                      <CuratedRecommendationGroup title="books" group="books" items={report.curatedRecommendations?.books ?? []} mode={mode} />
                      <CuratedRecommendationGroup title="films" group="films" items={report.curatedRecommendations?.films ?? []} mode={mode} />
                      <CuratedRecommendationGroup title="music" group="music" items={report.curatedRecommendations?.music ?? []} mode={mode} />
                    </div>
                    {report.recommendations.length ? (
                      <div className="space-y-4 border-t border-white/8 pt-2">
                        <p className="text-[11px] tracking-[0.32em] text-stone-500 uppercase">portable practices</p>
                        <div className="grid gap-4 md:grid-cols-2">
