...
function getQuestionPrompt(questionId: string) {
  return fullLiteQuestionSet.find((question) => question.questionId === questionId)?.prompt ?? questionId;
}

function makeEvidence(session: TestSession, vector: FeatureVector, ref: EvidenceRef): EvidenceTrace {
  const featureScore = ref.feature ? vector.values[ref.feature] : undefined;

  return {
    questionId: ref.questionId,
    prompt: getQuestionPrompt(ref.questionId),
...
function buildEvidence(session: TestSession, vector: FeatureVector, refs: EvidenceRef[]) {
  return refs.map((ref) => makeEvidence(session, vector, ref));
}
...
function getAestheticReading(session: TestSession, vector: FeatureVector): DimensionReading {
  ...
  if (feature === "aesthetic_music_nocturnal") {
    return {
      ...
      evidence: buildEvidence(session, vector, [
...
function getEmotionalReading(session: TestSession, vector: FeatureVector): DimensionReading {
...
function getTemporalReading(session: TestSession, vector: FeatureVector): DimensionReading {
...
function buildOverview(session: TestSession, vector: FeatureVector) {
...
function buildTensions(session: TestSession, vector: FeatureVector) {
...
function buildArchetype(session: TestSession, vector: FeatureVector): Archetype {
...
export function buildReport(session: TestSession, vector: FeatureVector): ReportPayload {
  const safetyFlags = collectSafetySignals(session, vector);
  const dimensionReadings = [
    getAestheticReading(session, vector),
    getEmotionalReading(session, vector),
    getTemporalReading(session, vector),
  ];
  const tensions = buildTensions(session, vector);
  const archetype = buildArchetype(session, vector);
  ...
  return {
    ...
    overview: buildOverview(session, vector),
    dimensionReadings,
    tensions,
    archetype,
    recommendations,
